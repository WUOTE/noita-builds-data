...
function add_icon_above_head(game_effect_entity, icon_file, event )
...
-- event voting
local streaming_config_allowed_event_types =
{
	true, --STREAMING_EVENT_AWFUL = 1
	true, --STREAMING_EVENT_BAD = 2
	true, --STREAMING_EVENT_NEUTRAL = 3
	true, --STREAMING_EVENT_GOOD = 4
}
...
function _streaming_get_event_for_vote()
	...
	if ( #weighted_list == 0 ) then
		print( "No valid events, picked NOTHING" )
		return "NOTHING", "$streamingevent_nothing", "$streamingeventdesc_nothing", "data/ui_gfx/streaming_event_icons/nothing.png"
	end
