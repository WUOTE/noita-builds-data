function can_run()
	return new_world_entered and math.abs(new_world_x) > 500 and new_world_y < 50 and Random(0,2) == 0
end

function run()
	if (new_world_biome == "desert") then
		spawn()
	end
end
...
function spawn()
	...
	if ( found and is_sector(x, SECTOR_ANIMAL) ) then
		id = EntityLoad( "data/entities/animals/scorpion.xml" )
