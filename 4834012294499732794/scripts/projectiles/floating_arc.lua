...
-- TODO: raycast hits everything which gives messy results with some projectiles that emit smoke etc.
local ray_success,r_x,r_y = Raytrace( pos_x, pos_y, pos_x, pos_y + ray_distance )
if not ray_success then return end

-- adjust velocity so that projectile steers towards target height
local vel_x,vel_y = ComponentGetValueVector2( velocity_comp, "mVelocity", vel_x, vel_y)
local pvel_y = vel_y
