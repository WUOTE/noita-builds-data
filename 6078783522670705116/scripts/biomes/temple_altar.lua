...
function spawn_all_shopitems( x, y )
	...
	if( Random( 0, 100 ) <= 50 ) then
		for i=1,count do
			...
			generate_shop_item( x + (i-1)*item_width, y - 24, false, nil, true )
			LoadPixelScene( "data/biome_impl/temple/shop_second_row.png", "data/biome_impl/temple/shop_second_row_visual.png", x + (i-1)*item_width - 8, y-16, "", true )
		end
	else	
		for i=1,count do
			if( i == sale_item_i ) then
