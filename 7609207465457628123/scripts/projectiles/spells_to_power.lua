...
if ( who_shot ~= nil ) and ( comp ~= nil ) then
	for i,projectile_id in ipairs(projectiles) do
		if ( projectile_id ~= root_id ) and ( projectile_id ~= entity_id ) then
	...
	exprad = math.floor( exprad + math.log( totalcount * 10.5 ) )
	...
	edit_component( effect_id, "ParticleEmitterComponent", function(comp3,vars)
		local part_min = math.min( math.floor( totalcount * 0.5 ), 120 )
		local part_max = math.min( totalcount + 1, 140 )
