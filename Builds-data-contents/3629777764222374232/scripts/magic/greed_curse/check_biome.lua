...
if ( status == 0 ) then
	if ( string.find( biome, "holymountain" ) == nil ) and ( string.find( biome, "victoryroom" ) == nil ) and ( string.find( biome, "boss_arena" ) == nil ) then
		status = 1
		timer = 0
	end
elseif ( status == 1 ) then
	if ( ( string.find( biome, "holymountain" ) ~= nil ) or ( string.find( biome, "victoryroom" ) ~= nil ) or ( string.find( biome, "boss_arena" ) ~= nil ) ) and ( old_depth < depth ) and ( depth_remainder > 150 ) then
		--[[
...
if ( status == 1 ) then
	if ( timer < 20 ) then
		...
		GamePrint( "$log_greed_curse_back" )
		--EntityLoad( "data/entities/misc/greed_curse/greed_ghost_portal.xml", pos_x, pos_y - 32 )
