...
if ( #players > 0 ) then
	...
	if ( string.len(note) > 0 ) then
				...
				for i,comp in ipairs(variables) do
					...
					if ( name == "kantele_song_pos" ) then
						...
						if ( #b > 0 ) then
							...
							print( currsong .. ", " .. current_note .. ", " .. note )
		...
		if ( do_portal_magic ) then
			local already_done = GameHasFlagRun( "kantele_secret_00" )
		...
		if ( do_bomb_magic ) then
			local already_done = GameHasFlagRun( "kantele_secret_01" )
		...
		if ( do_worm_magic ) then
			local already_done = GameHasFlagRun( "kantele_secret_02" )
