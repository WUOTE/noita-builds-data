...
local STREAMING_EVENT_AWFUL = 0
local STREAMING_EVENT_BAD = 1
local STREAMING_EVENT_NEUTRAL = 2
local STREAMING_EVENT_GOOD = 3
local STREAMING_EVENT_AUTHOR_NOLLAGAMES = "Nolla Games"
...
streaming_events =
{
		...
		action = function(event)
			...
			for a,player in ipairs( players ) do
				if EntityGetComponent(player, "GenomeDataComponent") ~= nil then
					local count = ProceduralRandom(GameGetFrameNum(),0,1,4)
					...
					if StreamingGetSettingsGhostsNamedAfterViewers() then
						add_icon_in_hud( entity_id, "data/ui_gfx/status_indicators/tiny_ghost.png", event, random_viewer_names )
					end
				end
			end
			-- TODO: remove ghosts (based on tag) after some time
		end,
	},
	{
		id = "TINY_GHOST_ENEMY",
	...
	},
	{
		...
		action = function(event)
			for i,entity_id in pairs( get_enemies_in_radius(400) ) do
				...
				EntityAddRandomStains( entity_id, CellFactory_GetType("alcohol"), 400 )
			end
		end,
	},
	{
	...
	},
	{
		...
		kind = STREAMING_EVENT_BAD,
		action = function(event)
			for i,entity_id in pairs( get_enemies_in_radius(600) ) do
				local x, y = EntityGetTransform( entity_id )
				
				local itempickup = EntityGetComponent( entity_id, "ItemPickUpperComponent" )
...
-- fix these
-- [X] LUA Error in (data/scripts/streaming_integration/event_list.lua): ComponentSetValue2 - couldn't find field DamageModelComponent.max_hp_old
-- [X] LUA Error in (data/scripts/streaming_integration/event_list.lua): ComponentSetValue2 - couldn't find field DamageModelComponent.mLastMaxHpChangeFrame
-- [X] XML parsing error in data/scripts/streaming_integration/entities/shield.xml - Couldn't create component by the name of: LifeTimeComponent (should be LifetimeComponent?)
-- [-] LUA: _add_extra_modifier_to_shot() - function 'explosive_projectile' not found in gun_extra_modifiers.lua	
-- [X?] fireball thrower lasts too long, at normal health it's an almost certain death
-- [X] invisibility either doesn't work, or the description doesn't tell that no stains are a requirement
-- [?] add UIIconComponent when a 'status' kind of effect is applied (e.g. drunk), or implement effect via EntityIngestMaterial if possible
-- [X] nukes everywhere ei n�ytt�ny toimivan (mit��n ei tapahtunu omille kudeille)
-- [ ] bomb rain event causes assert fails
