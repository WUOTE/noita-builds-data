...
if ( owner_id ~= nil ) and ( owner_id ~= NULL_ENTITY ) and ( #targets > 0 ) then
	...
	while ( limit < #targets ) and ( found == false ) do
		...
		if ( v ~= entity_id ) and ( v ~= owner_id ) and ( comp ~= nil ) and ( EntityGetHerdRelation( v, owner_id ) >= 90 ) then
			target = v
			found = true
			break
		end
	...
	if ( target ~= NULL_ENTITY ) then
		local eid = EntityLoad( "data/entities/particles/tinyspark_green_trail.xml", x, y )
