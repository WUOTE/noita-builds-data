...
function mod_setting_category_button( mod_id, gui, im_id, category )
	...
	local clicked2 = GuiImageButton( gui, im_id, 0, 0, "", image_file )
	if is_visible_string( category.ui_description ) then
		GuiTooltip( gui, category.ui_description, "" )
	end
	...
	for i,setting in ipairs(settings) do
		if setting.category_id ~= nil then
			...
			if setting.foldable then
				local clicked_category_heading = mod_setting_category_button( mod_id, gui, im_id, setting )
