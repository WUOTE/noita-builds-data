...
elseif t == throw_time + time_spawn then
	...
	local hp = ComponentGetValue2(comp, "value_float") * 0.6 -- max 60% healing
	...
	local hp = hp / projectile_count -- hp per shot
	print("heal per projectile: " .. hp)
	for i=1,projectile_count do
		local e = EntityLoad("data/entities/projectiles/healshot_safe_haven.xml", pos_x, pos_y - 10)
