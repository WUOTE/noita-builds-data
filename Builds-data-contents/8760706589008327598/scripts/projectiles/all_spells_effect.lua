...
SetRandomSeed( x * state, y * GameGetFrameNum() )
...
elseif ( state == 7 ) then
	for i=0,3 do
		...
		shoot_projectile( entity_id, "data/entities/projectiles/orb_poly.xml", x + ox, y + oy, math.cos( angle_ ) * 200, 0 - math.sin( angle_ ) * 200 )
	end
elseif ( state == 8 ) then
	for i=0,15 do
		local angle_ = math.pi * 0.125 * i
...
elseif ( state == 11 ) then
	...
	edit_component( eid, "LuaComponent", function(comp,vars)
